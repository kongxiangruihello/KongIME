...
out=root/'release/KongIME-0.26';out.mkdir(parents=True,exist_ok=False)
stage=root/'client/build/payload-v026';stage.mkdir(exist_ok=False)
...
for name in ['app.py','core.py','workflow.py','jobs.py','quick.py','profile_backup.py','cloud_client.py','library_tools.py','personal_data.py','local_snapshots.py','learning.py','restore_review.py','phrase_tools.py','engine_check.py','upgrade_check.py','complete_backup.py','update_check.py']:shutil.copy2(root/name,manager/name)
...
info.update(CFBundleShortVersionString='0.26.0',CFBundleVersion='260',CFBundleDisplayName='KongIME设置',CFBundleName='KongIME',LSUIElement=True);p.write_bytes(plistlib.dumps(info))
p=client/'Contents/Info.plist';info=plistlib.loads(p.read_bytes());info['KongIMEVersion']='0.26.0';info['tsInputMethodIconFileKey']='rime.pdf';info['CFBundleVersion']='12600';info['CFBundleShortVersionString']='1.1.2-KongIME.0.26';info.pop('SUFeedURL',None);info['SUEnableAutomaticChecks']=False;p.write_bytes(plistlib.dumps(info))
...
subprocess.run(['pkgbuild','--root',str(stage),'--install-location','/Library/Input Methods','--identifier','local.kongime.inputmethod','--version','0.26.0','--component-plist',str(componentFile),'--scripts',str(root/'branding/installer-scripts'),str(root/'client/build/KongIME-component.pkg')],check=True)
build_installer.build(root/'client/build/KongIME-component.pkg',out/'安装KongIME.pkg',root/'client/build/installer-v026','0.26.0')
shutil.copy2(root/'README-0.26.md',out/'开始使用.txt')
source=out/'源码';source.mkdir()
for n in ['LearningTool.cpp','EngineCheck.cpp','Launcher.swift','CredentialStore.swift','ChooseFolder.swift','IMEStatus.swift','app.py','core.py','workflow.py','jobs.py','quick.py','profile_backup.py','cloud_client.py','library_tools.py','personal_data.py','local_snapshots.py','learning.py','restore_review.py','phrase_tools.py','engine_check.py','upgrade_check.py','complete_backup.py','update_check.py','sgpu_pinyin.json','build_integrated.py','build_installer.py','build_client.sh','README-0.26.md','VALIDATION-0.26.md']:shutil.copy2(root/n,source/n)
...
shutil.copytree(root/'client/build/installer-v026',source/'branding/installer')
archive=root.parent/'KongIME-0.26-Mac.zip'
