...
当前版本：**0.25.0 测试版**。支持 Apple Silicon Mac、macOS 13 及以上。

## 本版功能

- 可关闭的中英文切换提示，菜单栏保留单色 K。
- 右键屏蔽完整候选词，可在设置里恢复；全拼、简拼、混合拼音一致生效。
- 中文／英文分别设置成对标点；光标置于中间，退格删除空对。
- 一份文件迁移设置、词库、短语、候选偏好和学习词频；预览、校验与失败恢复。

使用步骤与边界见 [0.25 使用说明](README-0.25.md)。测试结果见 [验证记录](VALIDATION-0.25.md)。
...
产物位于 `release/KongIME-0.25/`，上级目录同时生成 `KongIME-0.25-Mac.zip`。构建脚本要求输出目录尚不存在，避免覆盖已有交付物。未准备引擎依赖时，涉及真实引擎的测试不能运行。

开发设置界面可指定临时数据目录再启动 `app.py --no-open --port 0`；不要用开发测试覆盖真实输入法数据。

## 验证范围

153 项 Python 回归：151 通过，2 项缺少外部词库样本而跳过。真实 Rime／LevelDB 和原生 NSTextView 测试通过。未将 0.25 安装到当前运行环境；第三方编辑器兼容性和两台物理 Mac 的完整迁移仍待现场验证。
